...
// Banka listeleme
$banka_liste = $baglanti->query("SELECT * FROM banka ORDER BY tarih DESC")->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['ekle'])) {
    ...
    // Banka işlemi
    $banka_ekle = $baglanti->prepare("INSERT INTO banka (cari_id, tutar, islem_tipi, aciklama, tarih) VALUES (?, ?, ?, ?, ?)");
    $banka_ekle->execute([$cari_id, $tutar, $islem_tipi, $aciklama, $tarih]);
...
<div style="margin-left: 240px; padding: 20px;">
    ...
    <table border="1" cellpadding="8" cellspacing="0" style="width:100%; background:#fff;">
        ...
        <?php foreach ($banka_liste as $b): ?>
            <tr>
                <td><?= $b['cari_id'] ?></td>
                <td style="color:<?= $b['islem_tipi'] == 'Giris' ? 'green' : 'red' ?>; font-weight: bold;">
                    <?= $b['islem_tipi'] ?>
                </td>
